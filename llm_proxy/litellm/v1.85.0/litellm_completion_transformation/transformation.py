    ...
    def transform_responses_api_tools_to_chat_completion_tools(
        ...
        for tool in tools:
            if tool.get("type") == "mcp":
                ...
                # Codex 0.135.0+ wraps MCP server tools as a namespace with nested
                # function tools.  vLLM (and other OpenAI-compatible backends) only
                # understand type="function", so we expand each nested tool into a
                # flat function tool whose name is "{namespace}__{tool_name}".
                # Codex matches the call back to the right MCP server by splitting on
                # the double-underscore prefix.
                namespace_name = tool.get("name") or ""
                for nested_tool in tool.get("tools") or []:
                    if not isinstance(nested_tool, dict):
                        continue
                    if nested_tool.get("type") != "function":
                        continue
                    tool_name = nested_tool.get("name") or ""
                    qualified_name = (
                        f"{namespace_name}__{tool_name}"
                        if namespace_name
                        else tool_name
                    )
                    parameters = dict(nested_tool.get("parameters") or {})
                    if not parameters or "type" not in parameters:
                        parameters["type"] = "object"
                    expanded_tool: Dict[str, Any] = {
                        "type": "function",
                        "function": {
                            "name": qualified_name,
                            "description": nested_tool.get("description") or "",
                            "parameters": parameters,
                            "strict": nested_tool.get("strict", False) or False,
                        },
                    }
                    chat_completion_tools.append(
                        cast(ChatCompletionToolParam, expanded_tool)
                    )
            elif (
