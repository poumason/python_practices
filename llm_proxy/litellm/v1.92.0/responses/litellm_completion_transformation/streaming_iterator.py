...
class LiteLLMCompletionStreamingIterator(ResponsesAPIStreamingIterator):
    ...
    def _queue_tool_call_delta_events(self, tool_calls: object) -> None:
        ...
        for tc in tool_calls:
            ...
            if call_id not in self._tool_args_by_call_id:
                ...
                _ns, _bare = _split_flat_mcp_tool_name(fn_name)
                ...
                if _ns is not None:
                    _item_fields["namespace"] = _ns
                event = OutputItemAddedEvent(
                    type=ResponsesAPIStreamEvents.OUTPUT_ITEM_ADDED,
                    output_index=output_index,
                    item=BaseLiteLLMOpenAIResponseObject(**_item_fields),
                )
    ...
    def _queue_final_tool_call_done_events(self, litellm_complete_object: ModelResponse) -> None:
        ...
        for tc in tool_calls:
            ...
            if is_new_tool_call:
                ...
                if _ns is not None:
                    _added_fields["namespace"] = _ns
                event = OutputItemAddedEvent(
                    type=ResponsesAPIStreamEvents.OUTPUT_ITEM_ADDED,
                    output_index=output_index,
                    item=BaseLiteLLMOpenAIResponseObject(**_added_fields),
                )
                event.__dict__["sequence_number"] = self._sequence_number
                self._pending_tool_events.append(event)
            ...
            if _ns is not None:
                _done_fields["namespace"] = _ns
            item_done_event = OutputItemDoneEvent(
                type=ResponsesAPIStreamEvents.OUTPUT_ITEM_DONE,
                output_index=output_index,
                sequence_number=self._sequence_number,
                item=BaseLiteLLMOpenAIResponseObject(**_done_fields),
            )
    ...
    def _default_response_created_event_data(self) -> dict:
        ...
        if "tools" in self.responses_api_request:
            # Flatten any namespace tools before echoing back in response.created so
            # that clients (e.g. Codex) don't try to re-expand the namespace block
            # themselves and produce wrong tool names.
            raw_tools = self.responses_api_request["tools"] or []
            flattened_tools, _ = LiteLLMCompletionResponsesConfig.transform_responses_api_tools_to_chat_completion_tools(
                raw_tools
            )
            response_created_event_data["tools"] = flattened_tools
        else:
