...
# Codex CLI registers MCP tools with a namespace/name split internally.
# When namespace tools are flattened (our request-side fix), the wire name
# becomes "<namespace><tool>" e.g. "mcp__my_server__read_file".  On the
# response side we must reverse this so Codex's tool router (which dispatches
# on ToolName{namespace, name}) can match the call against its registry.
#
# Codex namespace names are always "mcp__<server_name>__" where <server_name>
# is the MCP server key (may contain underscores).  The flat name is their
# concatenation: "mcp__<server_name>__<tool_name>".
#
# Split rule: greedily consume "mcp__" + one-or-more word chars + "__" as the
# namespace, leaving everything after as the bare tool name.  Using \w+ means
# server names with underscores (e.g. "my_server") are handled correctly.
#
# Examples:
#   "mcp__my_server__read_file"  ->  namespace="mcp__my_server__", name="read_file"
#   "mcp__svc__a__b"             ->  namespace="mcp__svc__",        name="a__b"
#   "exec_command"               ->  namespace=None,                 name="exec_command"
_MCP_FLAT_NAME_RE = re.compile(r"^(mcp__(?:(?!__).)+__)(.+)$")


def _split_flat_mcp_tool_name(flat_name: str) -> tuple[Optional[str], str]:
    """Split a flat MCP tool name back into (namespace, bare_name).

    Returns (None, flat_name) for non-MCP names so callers can use the
    result unconditionally.
    """
    m = _MCP_FLAT_NAME_RE.match(flat_name)
    if m:
    ...
    def transform_responses_api_tools_to_chat_completion_tools(
        ...
        for tool in tools:
            if tool.get("type") == "mcp":
                ...
                # Codex CLI wraps each MCP server's tools in a proprietary
                # {"type":"namespace","name":"mcp__<server>__","tools":[...]} block
                # when talking to the Responses API.  OpenAI's own backend unwraps
                # this server-side, but generic OpenAI-compatible backends (vLLM,
                # Ollama, LM Studio, etc.) do not understand the "namespace" type and
                # silently drop or reject the entire block, making MCP tools
                # unreachable.
                #
                # Fix: flatten each nested function tool into a top-level
                # ChatCompletionToolParam named "<namespace_prefix><tool_name>".
                # The namespace name already ends with "__" (e.g. "mcp__server__"),
                # so the flat name is simply their concatenation.  Codex's tool
                # router dispatches on the preserved name end-to-end, so no reverse
                # map is needed.
                namespace_prefix: str = tool.get("name") or ""
                nested_tools = tool.get("tools") or []
                for nested in nested_tools:
                    if not isinstance(nested, dict) or nested.get("type") != "function":
                        continue
                    nested_params = dict(nested.get("parameters", {}) or {})
                    if not nested_params or "type" not in nested_params:
                        nested_params["type"] = "object"
                    flat_tool: Dict[str, Any] = {
                        "type": "function",
                        "function": {
                            "name": namespace_prefix + (nested.get("name") or ""),
                            "description": nested.get("description") or "",
                            "parameters": nested_params,
                            "strict": nested.get("strict", False) or False,
                        },
                    }
                    chat_completion_tools.append(cast(ChatCompletionToolParam, flat_tool))
            elif tool.get("type") == "function":
                typed_tool = cast(FunctionToolParam, tool)
                # Support both Responses API format (name at top level) and Chat
                # Completions format (name nested inside "function"). The latter
                # occurs when our namespace-flattening code already converted the
                # namespace block into ChatCompletionToolParam structs and those
                # are passed back through this function.
                _fn_dict = typed_tool.get("function") if isinstance(typed_tool.get("function"), dict) else {}
                _name = typed_tool.get("name") or (_fn_dict.get("name") if _fn_dict else None) or ""
                _description = typed_tool.get("description") or (_fn_dict.get("description") if _fn_dict else None) or ""
                _strict = typed_tool.get("strict") or (_fn_dict.get("strict") if _fn_dict else None) or False
                # Ensure parameters has "type": "object" as required by providers like Anthropic
                parameters = dict(typed_tool.get("parameters", {}) or (_fn_dict.get("parameters", {}) if _fn_dict else {}) or {})
    ...
    def transform_chat_completion_tools_to_responses_tools(
        ...
        for tool in all_chat_completion_tools:
            if tool.type == "function":
                ...
                _flat_name = function_definition.name or ""
                _tool_namespace, _tool_name = _split_flat_mcp_tool_name(_flat_name)
